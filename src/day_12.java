 	...
 	public static void executeInstruction2(String instruction)
 	{
 		...
 		switch(instructionType)
 		{
	 		...
	 		case 'L':
	 		{
 				...
 				long res_x = Math.round( Math.cos(Math.toRadians(value)) * relative_x 
	 						- Math.sin(Math.toRadians(value)) * relative_y);
	 			long res_y = Math.round(Math.sin(Math.toRadians(value))* relative_x 
	 						+ Math.cos(Math.toRadians(value)) * relative_y);
